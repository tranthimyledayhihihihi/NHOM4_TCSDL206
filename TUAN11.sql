...
BEGIN
    ...
    BEGIN
        PRINT N'Dữ liệu không hợp lệ: Mã hàng đã tồn tại.';
        RETURN;
    END;
    ...
    BEGIN
        PRINT N'Dữ liệu không hợp lệ: Mã loại hàng không tồn tại.';
        RETURN;
    END;
    ...
    BEGIN
        PRINT N'Dữ liệu không hợp lệ: Mã công ty không tồn tại.';
        RETURN;
    END;
...
AS
BEGIN
    -- Kiểm tra mã mặt hàng có tồn tại trong bảng MATHANG
    ...
    -- Thống kê tổng số lượng hàng đã đặt
    ...
    IF @TongSoLuong = 0
    BEGIN
        PRINT N'Mặt hàng này chưa được đặt hàng.';
    END
    ELSE
    BEGIN
        PRINT N'Tổng số lượng hàng đã đặt của mặt hàng có mã ' + @MaHang + N' là: ' + CAST(@TongSoLuong AS NVARCHAR(50));
    END
END;
GO

-- Gọi thủ tục với mã mặt hàng cần thống kê
EXEC ThongKeSoLuongHangBan @MaHang = 'MH00000001';
...
from MATHANG;
INSERT INTO MATHANG
VALUES ('MH00000005', N'NOKIA', 'LH00000002', 'NC00000001', 100, N'Cái', 1000000);
INSERT INTO DONDATHANG 
VALUES ('HD00000005', 'KH00000003', 'NV00000001', '2009-05-10', '2009-05-15', '2009-05-18', N'234 Đường F');
INSERT INTO CHITIETDATHANG
VALUES ('HD00000005', 'MH00000005', 2000000, 2,  20000);
 -- Nếu tổng tồn kho cho phép
UPDATE CHITIETDATHANG
SET SOLUONG = 4
WHERE SOHOADON = 'HD00000001';
SELECT *FROM CHITIETDATHANG
UPDATE CHITIETDATHANG
SET SOLUONG = 10000
WHERE SOHOADON = 'HD00000001';
